...
#include <Adafruit_ST7789.h> // Hardware-specific library for ST7789 with functions required for ST7735
#include <SPI.h> // Used to communicate with the display
#include <math.h>
#include <TimeLib.h> // Used to keep offline time
#include <NTPClient.h> // Used to get online time
#include <WiFiUdp.h> // Required for NTPClient.h
#include <Wire.h> // Required to define SCL and SDA pins from the display
...
// Question arrays with corresponding Strings used for communication with Pi
int AG[20];
String AG1;
int SSN[20];
String SSN1;
...
String t1;
String h1;
// Array used for average marks per question area
...
int avgh[24] = {50,10,10,20,5,9,10,20,10,30,50,50,40,30,20,10,20,20,30,40,20, 40, 20};
...
int widthValue;
int newTimePart;
// Used for nonblocking buzzer timers
...
// Connect to wifi network on startup
void setup_wifi() {
  delay(10);
...
// Reconnect to broker if not connected to broker but connected to network (handled in loop)
void reconnect(){
  if (!client.connected()){
...
// Functions for displaying screens
...
// Funtion to display the current time at position x,y
...
// Function to display batterypercentage and notify when there is no battery connected
void dispCurPercentage(){
  tft.setCursor(3,4);
  tft.setTextColor(BACKCOLOR);
  percentage = (analogRead(35) - 1800) * 0.17;
  tft.fillRect(2,3,100,10,PRINTCOLOR);
...
// Function used to dynamicly change time to String time
...
// Function to draw questionbars in the parkinson monitor based on the position of the rotary encoder
...
// If the hour changes : update the hourly shown bars
void updateShowedBars(){
  drawBarChart(avgh[(newTime.substring(0,2)).toInt()], avgh[(newTime.substring(0,2)).toInt()+1], avgh[(newTime.substring(0,2)).toInt()+2], avgh[(newTime.substring(0,2)).toInt()+3], avgh[(newTime.substring(0,2)).toInt()+4], avgh[(newTime.substring(0,2)).toInt()+5]);;
  curHour = newTime.substring(0,2);
}

// Draw bars with average amount of movement of the past 5 days (averaging handled on Pi)
...
// Update ratios between kinds of movement
...
// Based on the current questionnumber, define the position of the bar to be drawn
...
// If the button is pressed, do what is meant to happen based on the displayed screen
void rotaryButton(){
  if (digitalRead(15) == LOW){
      ...
      if (millis() - startPress > 500){
        if (screen == 2){
          ...
          tft.print("uploaden.");
          inWalk = 0;
          t1.concat("Trillingen: 1,");
          for (int i = 1; i<24; i++){
            tl.concat(t[i]);
            tl.concat(",");
          }
          h1.concat("Bewegingen: 1,");
          for (int i = 1; i<24; i++){
            hl.concat(h[i]);
            hl.concat(",");
          }
...
// Manage what is displayed when the rotary encoder turns
void displayManager(){
  if ((counter != screen)){
    if (counter == 0){
      tft.fillScreen(PRINTCOLOR);
...
void loop() {
  if (millis() > time_current_amount + 30000 and inMonitor == 0){ // Wait 30 (non-blocking) seconds
    ...
    // Publish the amount of shakings and movements to /Pi/DosiMate
    client.publish("/Pi/DosiMate",String("Bewegingen: 1," + String(hl)).c_str());
    client.publish("/Pi/DosiMate",String("Trillingen: 1," + String(tl)).c_str());
